...
{
	bool was_active = animation_active;
	...
	/*
	 * Only repaint if an animation had actually overwritten the zones.
	 * animation_stop() is called on every zone write, and the restore below
	 * costs four WMI get/set pairs -- doing it when nothing was running made
	 * a single zone write five times more expensive than it needs to be.
	 */
	if (!was_active)
		return;
