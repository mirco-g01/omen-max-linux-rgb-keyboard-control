...
# hp_wmi is deliberately left alone. Upstream unloads and blacklists it,
# because the two drivers answer the same WMI GUID. On an OMEN MAX that is the
# wrong trade: hp_wmi -- or a patched build of it -- is what owns fan and power
# management, and taking it away leaves the GPU stuck at its floor. This driver
# has no fan or power code of its own to conflict with it (see README), so both
